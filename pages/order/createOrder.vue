		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view class="cen" v-if="addressData">
					<view class="top">
						<text class="name">{{ addressData.receiveName }}</text>
						<text class="mobile">{{ addressData.receivePhone }}</text>
					</view>
					<text class="address">{{ addressData.province }}-{{ addressData.city }}-{{ addressData.area }}-{{ addressData.detailAddr }}</text>
				</view>
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			...
			let id_nums=data1.id_nums.split(":");
			data1.goods.map(g=>{
				id_nums.find(id_num=>{
					let id_num_Arr= id_num.split("-");
					if(g.id==id_num_Arr[0]){
						let quantity= id_num_Arr[1];
						g.quantity=quantity;
						this.calcAmount.totalAmount+=quantity*g.goodsOriginalPrice;
						this.calcAmount.promotionAmount+=quantity*(g.goodsOriginalPrice-g.goodsPrice);
						this.calcAmount.freightAmount+=g.goodsTransfee;
						return true;
					}
					return false;
				})
			...
			this.cartPromotionItemList = data1.goods;
