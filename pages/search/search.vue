	...
	<view class="content">
		...
		<view class="search-keyword" @touchstart="blur">
			...
			<scroll-view class="keyword-box"  scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length > 0" v-show="!isShowKeywordList">
				...
				<view class="keyword-block">
					...
					<view class="keyword" >
						<view v-for="(item, index) in goodsList"  :key="index" @click="navToDetailPage(item)">{{ item.goodsName }}</view>
					</view>
				</view>
			</scroll-view>
		</view>
...
export default {
	...
	methods: {
		...
		loadHotKeyword() {
			...
			this.hotKeywordList = ['野生灵芝', '香菇', '笋干', '进口水果'];
		},
		//监听输入
		inputChange(event) {
			//兼容引入组件时传入参数情况
		...
		async	doSearch(key) {
			...
			let data =await  Api.apiCall('get', Api.goods.goodsList, params);
			if(data.code==0){
